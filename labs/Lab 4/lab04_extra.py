...
def flatten(lst):
    ...
    for i in lst:
        if type(i) == list:
            ...
            flat_list.append(i)
    return flat_list
    #return [i if type(i) != list else flatten(i) for i in lst]
...
def replace_elem(lst, index, elem):
    ...
    return [x if i != index else elem for i,x in enumerate(lst)]
...
def put_piece(board, max_rows, column, player):
    ...
    while r >= 0:
        if get_piece(board,r,column) == '-':
            new_board = [replace_elem(row, column, player) if idx == r else row for idx, row in enumerate(board)] #replace the row with a new row
            index = r
...
def make_move(board, max_rows, max_cols, col, player):
    ...
    -1
    """
    if (col < max_cols and col >= 0) and put_piece(board, max_rows, col, player)[0] >= 0:
        return (put_piece(board, max_rows, col, player)[0], put_piece(board, max_rows, col, player)[1] )
    else:
        return (-1, board)
