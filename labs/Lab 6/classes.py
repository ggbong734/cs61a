...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        opponent.hand = opponent.hand + [opponent.deck.draw() for _ in range(3)] 
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        store_index=[] #create an empty list to store index of discarded cards
        for card in opponent.deck.cards:
            if card.attack == other_card.attack or card.defense == other_card.defense:
                store_index.append(opponent.deck.cards.index(card))
        opponent.deck.cards = [card for index, card in enumerate(opponent.deck.cards) if index not in store_index] # create a new list of cards by excluding discarded cards
