...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if isinstance(self.ant, QueenAnt) and self.ant.is_queen:
                return  # True QueenAnt can't be removed, return nothing
            # Cannot remove QueenAnt that is contained by another ant
            if hasattr(self.ant, 'is_container') and self.ant.is_container \
                      and isinstance(self.ant.contained_ant, QueenAnt) and \
                                    self.ant.contained_ant.is_queen:
                return
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        if self.direction == "backward":      # realign direction for scared bees
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        test_place = self.place
        distance = 0
        while test_place.entrance is not None:
            if len(test_place.bees) > 0 and distance <= self.max_range and distance >= self.min_range: 
                return random_or_none(test_place.bees)
            else:
                test_place = test_place.entrance
                distance += 1
        return None   # no bee found within range
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor <= 0:
            for bee in list(self.place.bees): #created a copy of list of bees so iteration does not skip bees
...
class HungryAnt(Ant):
    ...
    # BEGIN Problem 6
    food_cost = 4
    time_to_digest = 3    # number of turns it takes to digest
    implemented = True  # Change to True to view in the GUI
    # END Problem 6
    ...
    def __init__(self, armor=1):
        ...
        self.digesting = 0  # number of turns left to digest
        # END Problem 6
    ...
    def action(self, colony):
        ...
        if self.digesting > 0:
            self.digesting -= 1
        elif self.digesting == 0 and len(self.place.bees) > 0: #eats a bee when ant is not digesting and a bee is present
...
# The WallAnt class
class WallAnt(Ant):
    """ WallAnt does nothing each turn but has high armor"""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower behaves like ThrowerAnt except it is watersafe"""
...
# END Problem 13
    ...
    food_cost = 7
    implemented = True   # Change to True to view in the GUI
    is_queen = True    # class attribute that is changed once the first queen is deployed
    # END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        "*** YOUR CODE HERE ***"
        Ant.__init__(self, armor)
        self.is_queen = QueenAnt.is_queen #instance attribute to know if real or impostor
        QueenAnt.is_queen = False         #change class attribute to False for next queen
        # END Problem 13
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if self.is_queen == False:              # QueenAnt is an impostor
            self.reduce_armor(self.armor)
        else:                                   # action taken by real QueenAnt
            ThrowerAnt.action(self,colony)      # First, QueenAnt follows action of ThrowerAnt
            current_place = self.place.exit     # Then, QueenAnt doubles the damage of all ants behind it
            while current_place.exit is not None:
                if current_place.ant:
                    if current_place.ant.damage_doubled is False:
                        current_place.ant.damage *= 2
                        current_place.ant.damage_doubled = True
                    if isinstance(current_place.ant, BodyguardAnt):
                        if current_place.ant.contained_ant is not None and \
                        current_place.ant.contained_ant.damage_doubled is False:
                            current_place.ant.contained_ant.damage *= 2
                            current_place.ant.contained_ant.damage_doubled = True
...
def make_slow(action, bee):
    ...
    def slow_action(colony):
        if colony.time % 2 == 0 and bee.armor > 0:
...
def make_scare(action, bee):
    ...
    def scare_action(colony):
        ...
        # Scared bees move backwards and do not attack ants
        if bee.place.ant and bee.place.entrance is not None:
            bee.move_to(bee.place.entrance)
        elif bee.place.ant == None and bee.place.entrance is not None:
            return action(colony)
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    old_bee_action = bee.action
    def new_action(colony):
        nonlocal duration
        if duration > 0:
            duration -= 1
            return effect(old_bee_action, bee)(colony)
        else:
            bee.direction = "forward"   # return direction for bees that were scared
            return old_bee_action(colony)
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if target and target.has_been_scared == False:
