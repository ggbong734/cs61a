...
def make_fib():
    ...
    def fib():
        ...
        if counter == 1:
            ...
            new = fib_list[counter-2] + fib_list[counter-3]
            fib_list.append(new)
        return new
...
def make_withdraw(balance, password):
    ...
    wrong_passwords = list()
    wrong_counter = 0
    
    def withdraw(amount, password_try):
        
        nonlocal balance
        nonlocal wrong_counter
        
        if wrong_counter == 3:
            return "Your account is locked. Attempts: ['{0}', '{1}', '{2}']".format(wrong_passwords[0], wrong_passwords[1], wrong_passwords[2])
        
        if password_try != password:
            wrong_counter = wrong_counter + 1
            wrong_passwords.append(password_try)
...
def make_joint(withdraw, old_password, new_password):
    ...
    """
    "*** YOUR CODE HERE ***"
    error_msg = withdraw(0, old_password) #if the right password, error_msg will be an int
    if type(error_msg) == str: #Wrong password entered to make joint account
        return error_msg
    
    #If old_password is correct
    def j_withdraw(amt, j_password_try):
        if j_password_try == new_password or j_password_try == old_password:
            return withdraw(amt, old_password)
        else:
            return withdraw(amt, j_password_try) 
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert (lower_bound(y)>0 and upper_bound(y)>0) or (lower_bound(y)<0 and upper_bound(y)<0), "Interval of divisor cannot span zero"
...
def multiple_references_explanation():
    return """par 1 will return a more accurate number compared to par 2 because
              there are less division operators in the formula which leads
              to less rounding errors. The multiple reference in this case is
