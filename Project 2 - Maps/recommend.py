...
def find_closest(location, centroids):
    ...
    closest = min(distance(c,location) for c in centroids)
    i = 0
    while i < len(centroids):
        if distance(centroids[i],location) == closest:
            closest = centroids[i]
        i= i+1
...
def group_by_centroid(restaurants, centroids):
    ...
    closest_centroid = [find_closest(loc,centroids) for loc in locations]
    centroid_name_pair = []
    i = 0
    while i < len(restaurants):
        pair =[closest_centroid[i], restaurants[i]]
        centroid_name_pair.append(pair)
        i = i + 1
    
    #alternative solution for creating pairs
    # centroid_name_pair = [[closest_centroid[i], restaurants[i]] for i in range(len(locations))]
    
    return group_by_first(centroid_name_pair) # return list of restaurants for EACH centroid 
    # END Question 4
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        "*** YOUR CODE HERE ***"
        clusters = group_by_centroid(restaurants, centroids) #returns clusters of restaurants
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** YOUR CODE HERE ***"
    pred_r_squared_pair = [find_predictor(user, reviewed, feature) for feature in feature_fns]
    best_predictor_pair = max(pred_r_squared_pair, key=lambda x: x[1])
